"""Codex CLI Host Dispatch Adapter (Milestone R11).

Strictly stdlib-only. Implements HostDispatchPort for OpenAI Codex CLI headless dispatch.
"""
...
class CodexDispatchAdapter(HostDispatchPort):
    """Host dispatch adapter for OpenAI Codex CLI environment."""
    ...
    def can_dispatch(self, envelope: DelegationEnvelope) -> Tuple[bool, Optional[str]]:
        ...
        if token_estimate > self._capabilities.max_token_context:
            return False, f"Instruction length exceeds Codex context limit ({self._capabilities.max_token_context})"
        return True, None
    ...
    def dispatch(
        ...
        if not can:
            return HostDispatchResult(
                ...
                host_execution_id=f"codex-unsup-{uuid.uuid4().hex[:8]}",
        ...
        exec_id = f"codex-proc-{uuid.uuid4().hex[:8]}"
        evidence = {
            "host_runtime": "codex",
            "handle": exec_id,
    ...
    def check_status(self, host_execution_id: str) -> HostStatusResult:
        return HostStatusResult(
            ...
            status=DispatchStatus.DISPATCHED,
            is_alive=True,
            details={"host": "codex", "handle": host_execution_id},
        )
